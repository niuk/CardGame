...
function getSrc(card) {
    if (card === util_1.Util.Joker.Big) {
        ...
        return `assets/${util_1.Util.Suit[suit]}${rankString}.png`;
    }
}
function renderHand() {
    ...
    if (alpha !== undefined && alpha > 0) {
        ...
        try {
            ...
            if (peekElement !== null && peekElement instanceof HTMLImageElement) {
                const peekEvent = new MouseEvent('click', {
                    button: event.button,
...
interactjs_1.default(".interactable").draggable({
    listeners: {
        start(event) {
            x = event.x0;
            y = event.y0;
            ...
            if (dragging || Math.sqrt(dx * dx + dy * dy) > 10) {
                ...
                for (let i = 0; i < hand.length; ++i) {
                    ...
                    if (selected) {
                        ...
                        if (cardImg === null || cardImg.parentElement === null) {
                            throw new Error();
                        }
