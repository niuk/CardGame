...
const playerName = util_1.Util.getCookie('playerName');
...
let gameStateMessage = undefined;
...
ws.onmessage = ev => {
    ...
    if ('errorDescription' in obj && typeof obj.errorDescription === 'string') {
        window.alert(obj.errorDescription);
    }
    else {
        gameStateMessage = obj;
...
function renderDeck(deckCount) {
    ...
    try {
        ...
        for (let i = 0; i < deckCount; ++i) {
            const cardImage = cardImages.get('Back0');
            context.drawImage(cardImage, i * gap, i * gap, cardWidth, cardHeight);
        }
    }
    finally {
        context.restore();
...
function renderOtherPlayers(playerIndex, otherPlayers) {
    context.save();
    try {
...
function renderOtherPlayer(index, otherPlayer) {
    if (otherPlayer === undefined) {
        return;
    }
    const cardImage = cardImages.get(`Back${index}`);
    for (let i = 0; i < otherPlayer.cardCount; ++i) {
        context.drawImage(cardImage, canvas.width / 2 + (i - otherPlayer.cardCount / 2) * gap - cardWidth / 2, 0, cardWidth, cardHeight);
    }
}
function binarySearch(haystack, needle) {
    return binary_search_1.default(haystack, needle, (a, b) => a - b);
}
class Vector {
    constructor() {
        this.x = 0;
...
function renderPlayer(deltaTime, cards) {
    context.save();
    try {
        ...
        for (let i = 0; i < index; ++i) {
            let cardImage = getCardImage(cards[i]);
...
function getCardImage(card) {
    if (card === util_1.Util.Joker.Big) {
        return cardImages.get('ColorJoker');
    }
    else if (card === util_1.Util.Joker.Small) {
